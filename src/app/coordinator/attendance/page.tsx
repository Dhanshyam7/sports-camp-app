...
export default async function CoordinatorAttendancePage({
  ...
  const [roster, timing, attendanceDay, history] = await Promise.all([
    ...
    getTimingForDate(assignment.sportId, date),
    ...
    <div className="space-y-6">
      <div className={`${glassCard} ${glassPanelPad}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <AttendanceDatePicker date={date} basePath="/coordinator/attendance" />
        </div>
      </div>

      <EditableRoster sportId={assignment.sportId} date={date} roster={roster} window={window} />
              ...
              {history.map((day) => (
                <tr key={day.date.toISOString()} className={tableRow}>
                  <td className={tableCell}>
                    <Link
                      href={`/coordinator/attendance?date=${toDateInputValue(day.date)}`}
